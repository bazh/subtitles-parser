var parser = (function() {
    ...
    pItems.fromSrt = function(data, options) {
        var useMs = options.ms ? true : false;
        var getHearingImpaerSubtitles = options.getHearingImpaerSubtitles || false;
        ...
        for (var i = 0; i < data.length; i += 4) {
            if(getHearingImpaerSubtitles) {
                ...
                if(data[i + 3].trim().indexOf('[') > -1 || data[i + 3].trim().indexOf(']') > -1) {
                    continue;
                } else {
                    items.push({
                        id: data[i].trim(),
